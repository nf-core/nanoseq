...
- Replaced `medaka` with `clair3` for small variants calling.

### Parameters

- Added
- Added
- Removed `--split_mnps` as `medaka` is no longer supported
- Removed `--phase_vcf` as `medaka` is no longer supported
