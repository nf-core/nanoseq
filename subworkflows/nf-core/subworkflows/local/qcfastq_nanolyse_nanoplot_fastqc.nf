 ...
 * Optional - remove Lambda reads with NanoLyse
 */
...
workflow QCFASTQ_NANOLYSE_NANOPLOT_FASTQC {
    take:
    ch_fastq
    skip_nanoplot
    skip_nanolyse
    ...
    /*
     * Remove Reads mapping to Lambda with NanoLyse
     */
    nanolyse_log     = Channel.empty()
    nanolyse_version = Channel.empty()


    if (!skip_nanolyse){
        NANOLYSE ( ch_fastq )
        //nanoplot_png     = NANOPLOT.out.png
        nanolyse_log     = NANOPLOT.out.log
        nanolyse_version = NANOPLOT.out.versions
    }
