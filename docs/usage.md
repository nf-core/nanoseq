...
| `fasta`         | Genome/Transcriptome fasta file for alignment. This can either be blank, a local path, or the appropriate key for a genome available in [iGenomes config file](../conf/igenomes.config). Must have the extension ".fasta", ".fasta.gz", ".fa" or ".fa.gz". |
| `gtf`           | GTF reference annotation file for alignment. This can either be blank or a local path. Must have the extension ".gtf" or ".gtf.gz". |
