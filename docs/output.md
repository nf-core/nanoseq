...
_Description_:
Nanoseq is designed to deal with various input data types. Pre-basecalled FASTQ files can be demultiplexed using _qcat_. The `--skip_demultiplexing` parameter can be used to skip this step if it is not required. Please see [`usage.md`](usage.md#--input) for more details about the format of the input samplesheet.
...
_Description_:
_NanoPlot_ can be used to produce general quality metrics from the per barcode FASTQ files.
...
- Clair3:
  ...
  - `variant_calling/clari3/<SAMPLE>/full_alignment.vcf.gz`: zipped full alignment VCF file with small variants.
  - `variant_calling/clair3/<SAMPLE>/full_alignment.vcf.gz.tbi`: index for zipped full alignment VCF file with small variants.
  - `variant_calling/clair3/<SAMPLE>/merge_output.vcf.gz`: .
  - `variant_calling/clair3/<SAMPLE>/pileup.vcf.gz`: zipped pileup VCF file with small variants.
  - `variant_calling/clair3/<SAMPLE>/pileup.vcf.gz.tbi`: index for zipped pileup VCF file with small variants.
  - `variant_calling/clair3/<SAMPLE>/run_clair3.log`: log file for run_clair3.
...
The short variant and structural variant calling steps is skipped by using the `--skip_vc` and `--skip_sniffles` flags.
