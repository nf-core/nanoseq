...
def helpMessage() {
    ...
    Demultiplexing
      ...
      --guppy_gpu_runners_per_device [int]    Number of '--gpu_runners_per_device' used for guppy when using '--guppy_gpu' (default: 6)
      --guppy_cpu_threads_per_caller [int]    Number of '--cpu_threads_per_caller' used for guppy when using '--guppy_gpu' (default: 1)
...
if (!params.skip_demultiplexing) {
    ...
    summary['Guppy GPU Runners Per Device'] = params.guppy_gpu_runners_per_device ?: 'Unspecified'
    summary['Guppy CPU Threads Per Caller'] = params.guppy_cpu_threads_per_caller ?: 'Unspecified'
...
if (!params.skip_demultiplexing) {
    ...
    process Guppy {
        ...
        gpu_runners_per_device = params.guppy_gpu_runners_per_device ? "--gpu_runners_per_device $params.guppy_gpu_runners_per_device" : "--gpu_runners_per_device 6"
        cpu_runners_per_device = params.guppy_cpu_threads_per_caller ? "--cpu_threads_per_caller $params.guppy_cpu_threads_per_caller" : "--cpu_threads_per_caller 1"
        proc_options = params.guppy_gpu ? "--device $params.gpu_device --num_callers $task.cpus $cpu_threads_per_caller $gpu_runners_per_device" : "--num_callers 2 --cpu_threads_per_caller ${task.cpus/2}"
        """
        guppy_basecaller \\
            --input_path $run_dir \\
